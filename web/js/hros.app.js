			...
			$('#desk .desktop-container').off('mousedown', 'li:not(.add)').on('mousedown', 'li:not(.add)', function(e){
				...
				if(e.button == 0 || e.button == 1){
					...
					$(document).on('mousemove', function(e){
						cx = e.clientX;//e.clientX <= 0 ? 0 : e.clientX >= $(document).width() ? $(document).width() : e.clientX;
						cy = e.clientY;//e.clientY <= 0 ? 0 : e.clientY >= $(document).height() ? $(document).height() : e.clientY;
					}).on('mouseup', function(){
